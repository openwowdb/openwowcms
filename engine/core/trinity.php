<?ph
#################################################################
# This file is a part of OpenWoW CMS by www.openwow.c

#   Project Owner    : OpenWoW CMS (http://www.openwow.co
#   Copyright        : (c) www.openwow.com, 20
#   Credits          : Based on work done by AXE and Maverf
#   License          : GPL
################################################################
...
class User extends SessionUse

/*
* CoreSQL - Returns Core specific SQL strin
*
...
* Unstuck characte
**
3 => "UPDATE ".$config_data[$id.'-2'][0].'.'.TBL_CHARACTERS." INNER JOIN ".$config_data[$id.'-2'][0].".character_homebin
...
/*
* return expansion string for id 0-classic, 1-tbc, 2-wotlk, 3-cat
*
...
$split_realmname=explode('|',$config['engine_realmnames']);//we have data in arra
$split0=explode(';',$config['engine_char_dbs']); //DB1|REALM_PORT|DB1_HOST|DB1_USER|DB1_PASS or DB2|REALM_POR
foreach ($split0 as $key=>$split00

$split1=explode('|',$split00);//we have data in arra
...
/*
* getUserGM - Returns the result of logged in use
* gm premission value FROM wow accounts!!
* On error returns 0 witch mean
...
/*
* getUserInfo - Returns the result array from a mysq
* query asking for all information stored regardin
...
/* Return result array *
$dbarray = mysql_fetch_array($result)
//check for bans and set $dbarray['banned'] 1-yes or 0-n
...
/*
* confirmUserPass - Checks whether or not the give
* username is in the database, if so it checks if th
* given password is the same password in the databas
* for that user. If the user doesn't exist or if th
* passwords don't match up, it returns an error cod
...
$result = $db->query($q)
if(!$result || (mysql_numrows($result) < 1))
return 1; //Indicates username failur
...
return 0; //Success! Username and password confirme

else
return 2; //Indicates password failur



/*
* usernameTaken - Returns true if the username ha
...
/*
* usernameBanned - Returns true if the username ha
* been banned by INGAME. Function for website ban i
* located in mysql.ph
*
...
/*
* convertPass - Uses raw pass and hash it to correc
* format
*
...
* This function sends pure item, no questions ask, all point(MP,DP) reducing, filtering et
* must be done before this function is called
* shopid -> 0=vote, 1=donatio
* realmid-> 0=first realm in config (engine_char_dbs), 1, 2, etc..
* money  -> 0= no money is sent (this is by default), if money is stated there will be no item send, you have to send seperatel
*
...
$subject = preg_replace( "/[^A-Za-z0-9]/", "", str_replace(array("\n", "\""), "", $subject)); //no whitespace
$item = preg_replace( "/[^0-9]/", "", $item); //item i
$stack = preg_replace( "/[^0-9]/", "", $stack); //number of item
$realmid = preg_replace( "/[^0-9]/", "", $realmid); //item i
if ($item<>'') $item = " ".$item
...
*
$split0=explode(';',$config['engine_char_dbs']); //DB1|REALM_PORT|DB1_HOST|DB1_USER|DB1_PASS or DB2|REALM_POR
$realm_array=explode('|',$split0[$realmid]);//we have data in arra
...
$msgout[1]=fread($telnet,1024); // msg after user typed i
...
if ($item<>'' && $item<>'0')//send ite

//sendmail to RA consol
fputs($telnet, ".send items ".$playername." \"".$subject."\" \"".$text."\" ".$item."[:".$stack."]\n")
//Note: if stack is too large item will not be sen
$mailtext="Mail with item sent! No money was sent.";$moneytext=''

elseif ($money>'0' && $money<>'')//send mone
